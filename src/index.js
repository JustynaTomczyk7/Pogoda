...
async function init() {
  ...
  const dailyForecast = await getDailyForecastData(
    coordinates.lat,
    coordinates.lon
  );
...
function getDailyForecastData(lat, lon) {
  return fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=202e8f8809d686fbabf4154bdca7779a&units=metric&lang=pl`
  )
...
function getCurrentWeatherData(lat, lon) {
  return fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=202e8f8809d686fbabf4154bdca7779a&units=metric&lang=pl`
  )
    .then((response) => response.json())
    .then((data) => data);
}
...
function displayDailyForecast(data) {
  ...
  for (let i = 0; i < 4; i++) {
    const highestPredictTemp = days[i].reduce((prev, current) => {
      return prev.temp > current.temp ? prev : current;
    });

    const lowestPredictTemp = days[i].reduce((prev, current) => {
      return prev.temp < current.temp ? prev : current;
    });
    ...
    icons[i].setAttribute(
      "src",
      `http://openweathermap.org/img/wn/${highestPredictTemp.icon}@2x.png`
    );
  }
}
